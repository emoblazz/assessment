    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
	    ...
	    <i class="glyphicon glyphicon-flash"></i>
            Progress
            <a href="export1.php?cid=<?php echo $_REQUEST['cid'];?>&term=<?php echo $_REQUEST['term'];?>" class="btn btn-primary"><i class="glyphicon glyphicon-import"></i>Export</a>
		 ...
		 <div class="box">
		    ...
		    ?><form method="post" action="adjust.php"><input type="hidden" name="cid" value="<?php echo $cid;?>"><input type="hidden" name="term" value="<?php echo $term;?>">
			    ...
			    $rowat=mysqli_fetch_array($at);
			      ...
			      if($ap<>0)
			      {
			      echo "<input type='hidden' name='id[]' value='$rowat[grade_id]' class='col-md-6'>";
			      echo "<td>";
			      echo "<input type='text' name='grade[]' value='$rowat[grade]' class='col-md-6'></td>";
			      echo "<th>";			      
			      echo $at_ave;
			      echo "</th>";
			      }
			  ...
			  $asp=$rowc['assign']*100;
			      if($asp<>0)
			      {
			      echo "<input type='hidden' name='id[]' value='$row[grade_id]' class='col-md-6'>";
			      echo "<td><input type='text' name='grade[]' value='$row[grade]' class='col-md-6'></td>";
			      }
			...
			<?php $queryq=mysqli_query($con,"select * from grade where stud_id='$sid' and term='$term' and class_id='$cid' and type='Quiz'")or die(mysqli_error());
			  ...
			  <?php 
			    $qp=$rowc['quiz']*100;
			      if($qp<>0)
			      {
				echo "<input type='hidden' name='id[]' value='$rowq[grade_id]' class='col-md-6'>";
				echo "<td><input type='text' name='grade[]' value='$rowq[grade]' class='col-md-6'></td>";
				$iq++;
			...
			<?php $querye=mysqli_query($con,"select * from grade where stud_id='$sid' and term='$term' and class_id='$cid' and type='Exam'")or die(mysqli_error($con));
			  ...
			  <?php 
				...
				$exam_ave=number_format($e_ave*$rowc['exam'],2);
			      if($exp<>0)
			      {
				echo "<input type='hidden' name='id[]' value='$rowexam[grade_id]' class='col-md-6'>";
				echo "<td><input type='text' name='grade[]' value='$rowexam[grade]' class='col-md-6'></td>";
			...
			?>
			<td><button class="btn tn-block btn-primary pull-right" type="submit">Save</button></td>
                      </tr>
                    <?php
                   }
                     ?>  
