	...
	<div class="row">
            <div class="col-md-8">
              ...
              <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                  <li class="active"><a href="#tab_1" data-toggle="tab">Active Quizzes</a></li>
                  <li><a href="#tab_2" data-toggle="tab">Previous Quizzes</a></li>
			...
			<?php
			  include('../dist/includes/dbcon.php');
				$id=$_SESSION['sid'];
				$querymain=mysqli_query($con,"select * from quiz natural join class_quiz natural join class natural join enrol where stud_id='$id' and class_quiz_stat='Activated' and quiz_type='Quiz' order by quiz_id desc")or die(mysqli_error());
				  $countquiz=mysqli_num_rows($querymain);
				  if ($countquiz<1){echo "<h4 class='text-red'>No active quiz!</h4>";} 
			...
			<div class="box-body">
				...
				<p>Duration: <b><?php echo $rowmain['quiz_duration'];?></b></p>
				  ...
				  ?>
				  <p>Items: <b><?php echo $row4['count'];?></b></p>
				  <p>Points:<b><?php
				     $points=mysqli_query($con,"select *,SUM(points) as spoints from question where quiz_id='$qid' and (question_type='Multiple Choice' or question_type='Modified True or False' or question_type='True or False' or question_type='Identification')")or die(mysqli_error());	
					$row4=mysqli_fetch_array($points);
					$total=$row4['spoints'];
				    
				    $mpoints=mysqli_query($con,"select *,SUM(points) as mpoints from question natural join answer where quiz_id='$qid' and question_type='Enumeration'")or die(mysqli_error());	
			...
			<?php
			  include('../dist/includes/dbcon.php');
				$id=$_SESSION['sid'];
				$querymain=mysqli_query($con,"select * from quiz natural join class_quiz natural join class natural join enrol where stud_id='$id' and class_quiz_stat='Deactivated' and quiz_type='Quiz'")or die(mysqli_error());
				$countquiz=mysqli_num_rows($querymain);
				  if ($countquiz<1){echo "<h4 class='text-red'>No previous quiz!</h4>";} 
				  while($rowmain=mysqli_fetch_array($querymain))
					  {
					    $qid=$rowmain['quiz_id'];
			?>
			<div class="box box-widget">
				...
				<?php
				  $taken=mysqli_query($con,"select COUNT(*) as counttaken from quiz_result where stud_id='$id' and quiz_id='$qid' and equiv='0' and quiz_taken='0000/00/00'")or die(mysqli_error());
				    $row5=mysqli_fetch_array($taken);
				    if (($row5['counttaken']==0))
				    {
				      echo "<a class='btn btn-app text-red'>
		    ...
		    <?php
		    $query3=mysqli_query($con,"select * from quiz_result natural join quiz where stud_id='$id' and quiz_type='Quiz' order by quiz_result_id desc")or die(mysqli_error());
